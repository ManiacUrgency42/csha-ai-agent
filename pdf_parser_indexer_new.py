...
# Heading criteria
heading_fonts = {"AGOJFY+Calibri-Bold"}
heading_font_sizes = {14.0}

# Subheading criteria
subheading_fonts = {}
subheading_font_sizes = {}

# Body text criteria
body_fonts = {"AGOJFY+Calibri-Bold", "JEXLBG+Calibri", "ORIKTW+ArialMT", "RYWHJQ+TimesNewRomanPSMT"}
body_font_sizes = {11.0, 12.0}
...
def save_text_elements_to_json(
    ...
    def detect_runs(
        ...
        target_fonts: List[str] = None,
        target_sizes: List[float] = None,
    ) -> List[Tuple[int, int]]:
        """
        ...
        for idx, element in enumerate(elements):
            ...
            matches_font = (target_fonts is None or font in target_fonts)
            matches_size = (target_sizes is None or size in target_sizes)
    ...
    heading_runs = detect_runs(text_elements, target_fonts=heading_fonts, target_sizes=heading_font_sizes)
    for part_idx, (heading_start_idx, heading_end_idx) in enumerate(heading_runs):
        ...
        data['headings'].append({
            'heading_number': part_idx + 1,
            'heading_title': part_title,
            'text': '',
            'url': url,
            'page': text_elements[heading_start_idx]['page'],
            'subheadings': []
        })
    ...
    subheading_runs = detect_runs(
        text_elements,
        target_fonts=subheading_fonts,
        target_sizes=subheading_font_sizes,
    )
    ...
    for sub_start_idx, sub_end_idx in subheading_runs:
        ...
        filtered_text = ''.join(
            ...
            if w.get('fontname') in body_fonts and round(w.get('font_size', 0),1) in body_font_sizes
        ).strip()
...
def main() -> None:
    ...
    structured_output = "output/test.json"
    url = "https://www.ncbi.nlm.nih.gov/books/NBK22400/"

    characters = extract_text_with_attributes(pdf_file)
    save_text_elements_to_json(characters, structured_output, url)
